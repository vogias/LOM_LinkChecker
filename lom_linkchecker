...
public class ODS_linkchecker {
    ...
    public static Connection Get_Connection(String databaseName,String userName,String passWord) throws Exception
    {
        ...
        catch (SQLException e)
        {
            ...
            System.out.println("Please be sure that you've created the database, the username and password are correct, and you are using mysql DB");
            System.exit(1);
            throw e;
        }
        catch (Exception e)
    ...
    void checkLink(File folder, String dbName, String tbName,String userName,String passWord, File brokenFolder ) {
        File[] listOfFiles;
        ...
        try{
            ...
            for (int i = 0; i < fileNumber; i++) {
                    ...
                    String metadataURL=(GetURLFromXML(fileName,"technical","location").toString()).replace("'", "\\'");
                    if(metadataURL.equals("Error"))
                        metadataURL=(GetURLFromXML(fileName,"lom:technical","lom:location").toString());
                    if(metadataURL.equals("Error"))
                        metadataURL=(GetURLFromXML(fileName,"oai_dc:dc","dc:identifier").toString());
                    ...
                    if(metadataURL.equals("Error"))
                    {
                        try {
                            ...
                            //-------------------------Move files ----------------------------------
                            // File newFile =new File(brokenFolder.toPath()+"\\"+listOfFiles[i].getName());
                            File newFile =new File(brokenFolder.getPath(),listOfFiles[i].getName());

                            Files.move(listOfFiles[i].toPath(), newFile.toPath());
                            System.out.println("File=" +listOfFiles[i].toPath()+ " was moved to" +brokenFolder);
                            //----------------------------------------------------------------------
                        }
                        catch (Exception e) {
                    ...
                    } else
                    {
                        ...
                        if(result!=200){
                            try {
                                ...
                                //-------------------------Move files ----------------------------------
                                System.out.println("File source=" +listOfFiles[i].toPath());
    ...
    public static void main(String args[]) {
        ...
        ods_linkchecker.checkLink(metadataFolder,"linkchecker","log",username,password,brokenFolder);
    }
}
